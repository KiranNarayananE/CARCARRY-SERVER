...
export const getBookingDetails = async (req, res) => {
  try {
    ...
    let ratingDetails = await tripModel.aggregate([
      { $match: {
        "$and":[
          { driver: new mongoose.Types.ObjectId(bookingList.driver) },
          { rating: { $exists: true } }
        ]
      } },
      { $group: {
      ...
      { $group: {
        _id: null,
        ratings: { $push: { rating: "$_id", count: "$count", totalRating: "$totalRating" } },
        total: { $sum: "$count" },
        average: { $avg: "$_id" }
      } },
      { $project: {
        ...
        ratings: {
          $map: {
            ...
            in: {
              ...
              count: "$$rating.count",
              percent: { $multiply: [100, { $divide: [ "$$rating.count", "$total" ] }] },
              average: { $divide: [ "$$rating.totalRating", "$$rating.count" ] }
            }
          }
        },
        average: 1
    ...
    res.status(200).json({ Bookings: bookingList ,ratingDetails:ratingDetails});
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error!" });
  }
}
...
export const profileUpdate = async (req, res) => {
  try {
    const {id}=req.user
    const{name,email,phone,userName,address,city,state,zipcode}=req.body
   
    const start =  await UserModel.findOneAndUpdate({_id: new mongoose.Types.ObjectId(id)},{$set: {name,email,phone,userName,address,city,state,zipcode}},{new:true})

    res.status(201).json({ user: start });
  } catch (err) {
...
export const getBookingHistory = async (req, res) => {
  try {
    ...
    const bookingList = await tripModel.aggregate([
      { $match: { user: new mongoose.Types.ObjectId(id), bookingStatus: "Completed" } },
      { $lookup: { from: "drivers", localField: "driver", foreignField: "_id", as: "user" } },
      { $unwind: "$user" },
...
export const rateRide = async (req, res) => {
  try {
    ...
    const start =  await tripModel.findOneAndUpdate({_id: new mongoose.Types.ObjectId(id)},{$set: {rating}},{new:true})

    res.status(201).json({ trips: start });
  } catch (err) {
    res.status(500).json({ error: err.code });
    console.log(err.message);
  }
